...
Ejecución (después de instalar dependencias):
  pip install -r requirements.txt
  python asr_espanol_espana.py

El script automatiza:
  1. Descarga de corpus RTVE 2022 y Common Voice ES
  2. Extracción de características de audio (Mel filterbank 80 filtros)
  3. Entrenamiento del tokenizer BPE (6000 tokens, español peninsular)
  4. Entrenamiento del modelo Conformer (17 bloques, d_model=512)
  5. Decodificación (greedy + beam search beam=8)
  6. Evaluación WER/CER sobre hablantes peninsulares
"""
...
class SpanishBPETokenizer:
    ...
    def train(self, texts: List[str]) -> None:
        ...
        # <sos>, <eos>, <unk> son manejados por bos_id/eos_id/unk_id;
        # solo ponemos los tokens verdaderamente personalizados en user_defined_symbols.
        user_symbols = ",".join(["<blank>", "<num>", "<sil>"])

        # Solo forzar caracteres del español peninsular si realmente están en el corpus.
        # required_chars con chars ausentes hace que SPM falle con freq=0 durante BPE.
        ...
        # Calcular el vocab_size efectivo según el tamaño real del corpus.
        # Para corpus grandes (>1M chars, como Common Voice + RTVE), se usa el valor completo.
        # Para corpus pequeños (test/demo), se limita para evitar el crash de SPM.
        ...
        if total_chars >= 1_000_000:
            ...
            # Corpus muy pequeño (test/demo): solo chars + bigramas como cota superior
        ...
        if effective_vocab < self.vocab_size:
            log.warning(
                ...
                f"Con el corpus real (Common Voice + RTVE, >1M chars) "
        ...
        train_kwargs: Dict[str, Any] = dict(
            ...
            split_digits=False,
            # Preservar dígrafos comunes del español: ll, ch, rr
            split_by_unicode_script=False,
            # Permite vocab menor al objetivo si el corpus es pequeño
            hard_vocab_limit=False,
        )
        # Forzar chars peninsulares solo si están presentes en el corpus
        if present_required:
...
def download_common_voice_spain(data_dir: str = "data/common_voice") -> Optional[Any]:
    ...
    Excluye explícitamente muestras de México, Argentina, Colombia, etc.
    """
    ...
    try:
        ...
        for split in list(dataset.keys()):
            ...
            log.info(
                f"  {split}: {original:,} → {len(dataset[split]):,} muestras peninsulares"
            )
        return dataset
    except Exception as exc:
        log.warning(f"Common Voice no disponible: {exc}")
        return None
...
def download_rtve2022(data_dir: str = "data/rtve2022") -> Optional[str]:
    ...
    data_dir = pathlib.Path(data_dir)
    data_dir.mkdir(parents=True, exist_ok=True)
    done_file = data_dir / ".downloaded"

    if done_file.exists():
        log.info(f"RTVE 2022 ya disponible en {data_dir}")
        return str(data_dir)
    ...
    try:
        ...
        log.info(f"RTVE 2022 cargado desde HuggingFace: {list(dataset.keys())}")
        return str(data_dir)
    except Exception as exc:
        log.warning(f"HuggingFace RTVE falló: {exc}")
    ...
    try:
        ...
        r = requests.get(zenodo_url, stream=True, timeout=60)
        if r.status_code == 200:
            tar_path = data_dir / "rtve2022asr.tar.gz"
...
class SpanishASRDataset(torch.utils.data.Dataset):
    ...
    Compatible con Common Voice ES (filtrado España) y RTVE 2022.
...
def _hf_to_samples(hf_dataset: Any, split: str, source: str) -> List[Dict]:
    ...
    for row in hf_dataset[split]:
        text = row.get("sentence") or row.get("text") or row.get("transcription") or ""
        if not text.strip():
            continue
        samples.append({
            "audio": row.get("audio", {}),
...
def build_dataloaders(
    ...
    """
    Descarga (si necesario) y construye los DataLoaders de entrenamiento y
...
def main() -> None:
    ...
    Ejecuta el pipeline completo:
      1. Descarga datasets (RTVE 2022 + Common Voice ES)
      2. Entrena tokenizer BPE peninsular
      3. Construye DataLoaders
